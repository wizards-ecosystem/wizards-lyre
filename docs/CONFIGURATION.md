...
Lyre is configured entirely by environment variables. There is no config file
and no settings UI.
...
## Lyre's own variables

| Variable | Default | What it does |
|---|---|---|
| `LYRE_PORT` | `8421` | Port for the HTTP server. The host is always `127.0.0.1` and is not configurable — see [SECURITY.md](../SECURITY.md). |
...
| `LYRE_CHECKPOINTS_DIR` | `<repo>/checkpoints` | ACE-Step model weights. Must be a directory literally named `checkpoints` — upstream resolves weights as `<parent>/checkpoints/<name>`, and the worker fails loudly rather than silently looking in the wrong place. |
...
`dit_profile`, and a job may override it (SPEC.md §4.1):
