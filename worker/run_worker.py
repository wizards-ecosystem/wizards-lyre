...
until it actually holds the lease. That same lease's heartbeat thread also
keeps `worker_status.updated_at` fresh (`server.jobs.touch_worker_heartbeat`)
independent of the main poll loop -- which blocks inside a long-running job
-- so `server.jobs.get_worker_status` never mistakes a busy-but-alive worker
for a crashed one. On a clean shutdown (`stop_event` set), readiness is
explicitly published as unavailable so `/api/health` reflects the stop
immediately instead of waiting for that same staleness check to kick in.
"""
...
def _current_readiness(startup_ready: bool, startup_message: str) -> tuple[bool, str]:
    ...
    only when nothing has fully loaded successfully, so a genuine startup
    failure still reads as one instead of a generic "nothing loaded".

    Uses `is_fully_loaded()` (DiT *and* LM), not `get_loaded_dit_profile()`
    alone: a backend can commit a loaded DiT to its state before the LM
    finishes initializing, so checking the DiT alone would report `ready`
    while generation still can't run (a partial startup read as full
    recovery). Backends without the distinction (e.g. `worker.mock_worker`,
    which has no separate LM state) fall back to the DiT-loaded check."""
    module = jobs.resolve_worker_module()
    get_loaded = getattr(module, "get_loaded_dit_profile", None)
    loaded_dit_profile = get_loaded() if get_loaded is not None else None
    is_fully_loaded = getattr(module, "is_fully_loaded", None)
    fully_loaded = (
        is_fully_loaded() if is_fully_loaded is not None else loaded_dit_profile is not None
    )
    if fully_loaded:
